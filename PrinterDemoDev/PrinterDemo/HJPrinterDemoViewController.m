...
//    if (configure) {
//        [setting addEntriesFromDictionary:configure];
//    }
//    [setting setObject:@1 forKey:@"printertype"]; //打印机宽度80mm
//    [setting setObject:@1 forKey:@"pagecut"];//二等分纸，针式打印机
//    [PrinterSDK setPrinterSetting:setting];
//}
...
#warning 请用真机 否则会有编译错误
    ...
    [PrinterSDK setPrintFormat:1 LineSpace:28 alinment:0 rotation:0];// 1 小字体  ，28默认行间距,0左对齐
     [PrinterSDK addPrintText:@"\n"];
    [PrinterSDK addPrintText:[PrinterSDK getSplitLine]];
     [PrinterSDK addPrintText:@"\n"];
...
- (IBAction)printWithModel:(id)sender {
   NSArray *headers =@[@"编号",@"名称",@"价格",@"数量",@"小计金额"];
    ...
    NSArray* body =@[headers,values0,values1];
        ...
        [PrinterSDK printModel:model fromviewc:self printerTag:0 preview:YES failed:^(BOOL res) {
...
{
//    [PrinterSDK printModel:self.printmode fromviewc:self printerTag:self.printerTag preview:NO failed:self.choosePrinter];
     [PrinterSDK startPrint:self.navigationController deviceTag:0];
}
